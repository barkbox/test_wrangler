  ...
  describe '#show' do
    context 'when the named experiment exists' do
      ...
      it 'assigns the experiment' do
        get :show, {experiment_name: 'facebook_signup'}
        expect(assigns['experiment'].name).to eq('facebook_signup')
      end
      ...
      context 'when the experiment is assigned to cohorts' do
        ...
        it 'assigns the cohorts' do
          get :show, {experiment_name: 'facebook_signup'}
          expect(assigns['experiment']['cohorts'].first.name).to eq('facebook')
          expect(assigns['cohorts'].first.name).to eq('facebook')
        end
      end
    end
    ...
    context 'when the named experiment does not exist' do
      it 'responds with 404' do
        get :show, {experiment_name: 'random'}
