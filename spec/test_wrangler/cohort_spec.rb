...
describe TestWrangler::Cohort do
  ...
  describe '#critera' do
    context 'first call' do
      ...
      it 'raises an error if any of the matcher types are not valid' do
        cohort = TestWrangler::Cohort.new('mobile', [{type: :some_matcher, user_agent: [/Mobi/, /brows/]}])
  ...
  describe '==(other)' do
    it 'returns true if the other object is a Cohort and #serialize is equivalent' do
      cohort = TestWrangler::Cohort.new('mobile', [{type: :user_agent, user_agent: [/Mobi/, /snuffle/]}])
      cohort2 = TestWrangler::Cohort.new('mobile', [{type: :user_agent, user_agent: [/Mobi/, /snuffle/]}])
  ...
  describe '#serialize' do
    it "outputs a data structure that can be used for persistence" do
      cohort = TestWrangler::Cohort.new('mobile', [{type: :user_agent, user_agent: [/Mobi/, /snuffle/]}, {type: :cookies, cookies: [{'this' => 'that'}]}])
      expected = ['mobile', [{'type' => 'user_agent', 'user_agent' => [/Mobi/, /snuffle/]}.to_json, {'type' => 'cookies', 'cookies' => [{'this' => 'that'}]}.to_json ]]
